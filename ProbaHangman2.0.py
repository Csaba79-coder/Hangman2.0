...
"""def capitalize_function(guess_valid_letters):
    letters = list(string.ascii_letters)
    if guess_letters in letters or guess_letters.lower() in letters:
        return True
    else:
        print("Please guess a valid letter!")
        return guess_letters()

def guess_valid_letters(guess_letters):
    while True:
        guess_letters = input('What is your letter? ')
        if guess_letters == "quit":
            sys.exit()
        elif capitalize_function(guess_valid_letters):
            return guess_letters"""
...
def tries(lives):
    if lives >= 7:
        ...
        print(lose)
        while True:
            new_game = input('You have lost! Would you like another game? y = Yes n = No: ')
            if new_game == "y":
                valid_answer = True
                main()
            elif new_game == "n":
